...
//-----------------------------------------------------------------------------------------
#include "mcuf\CommandLineExecutor.h"
...
//-----------------------------------------------------------------------------------------
using mcuf::CommandLineExecutor;
 ...
 * @return false 
 */
bool CommandLineExecutor::addCommand(mcuf::CommandLineHandler& commandLineHandler){
  for(int i=0; i<32; ++i){
    if(this->mCommand[i] == nullptr){
 ...
 * @return false 
 */
bool CommandLineExecutor::removeCommand(mcuf::CommandLineHandler& commandLineHandler){
  for(int i=0; i<32; ++i){
 ...
 */
mcuf::CommandLineHandler* CommandLineExecutor::getCommand(const char* command){
  if(command == nullptr)
    return nullptr;
...
void CommandLineExecutor::commandExecute(void){
  mcuf::CommandLineHandler* c = this->getCommand(static_cast<const char*>(this->pointer()));
  if(c != nullptr)
